...
class BDCN(nn.Module):
    def __init__(self,in_channels=3, pretrain=None, logger=None, rate=4):
        super(BDCN, self).__init__()
        self.pretrain = pretrain
        t = 1

        self.features = vgg16_c.VGG16_C(in_channels,pretrain, logger)
...
if __name__ == '__main__':
    ...
    trans=PILToTensor()
    i=trans(img).unsqueeze(0).type(torch.float32)
    print(i.shape)
    a=model(i)[-1]
    c=(torch.sigmoid(a)>0.5).float()*255
    # c=np.array(a.detach().numpy())
    # print(c.shape)
