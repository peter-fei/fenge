...
import numpy as np
import math
import net.custom_functional as myF
...
def t():
    ...
    img=cv2.imread('../data_Naso_gtv/train/image/0.png')
    img2=cv2.imread('../data_Naso_gtv/train/label/0.png')
    plt.imshow(img)
    plt.show()
    img2=cv2.cvtColor(img2,cv2.COLOR_RGB2GRAY)
    print(img2.shape)
    canny_map=torch.from_numpy(img).float().unsqueeze(0).permute(0,3,1,2)
    resnet_map=torch.from_numpy(img2).float().unsqueeze(0).unsqueeze(0)
    canny_map_filters_in = 3
